    ...
    $.fn.tagEditor = function(options, val, blur) {
            ...
            selector.each(function() {
                ...
                if (options == 'getTags')
                    ...
                    if (!blur) ed.click();
                        ...
                        // // KZ: hide reset tags icon, show search icon
                        // show('search-icon');
                        // hide('reset-tags');
                        // // end KZ
                    }
        ...
        return selector.each(function() {
            ...
            ed.click(function(e, closest_tag) {
                ...
                $('.placeholder', ed).remove();
                // KZ: hide search icon, show reset tags icon
                if (tag_list.length > 0) {
                    hide('search-icon');
                    show('reset-tags');
                    // when one or more tags exist in the search then indicate dimension icons as active
                    enableDimensions('wrap');
                }
                // end KZ
            ...
            ed.on('click', '.tag-editor-delete', function(e) {
                // KZ: remove all keywords from cache
                cache.clear();
                changeAllHidden('false');
                removeContentDimension();
                ...
                tag.addClass('deleted').animate({
                    ...
                    li.remove();
                    set_placeholder();
                    // KZ: IF no tags THEN hide reset tags icon, show search icon
                    if (tag_list.length === 0) {
                        show('search-icon');
                        hide('reset-tags');
            ...
            ed.on('keydown', 'input', function(e) {
                ...
                // right/down key
                    ...
                    // KZ: onchange remove all keywords from cache
                    cache.clear();
                    changeAllHidden('false');
                    removeContentDimension();
